...
data = pd.read_csv("DJIA_Data_BUY_CSVcsv")
...
for i in range(len(data)):
    ...
    if current_date != previous_date:
        ...
        if last_daily_atr > 375:
            ...
            trade_open_hours = [("09:30:00", "11:30:00"), ("14:00:00", "15:57:00")]
    ...
    if within_trade_hours and not trade_open and daily_losses < 3:
        ...
        if data['Close'].iloc[i] > data['Open'].iloc[i] and data['Close'].iloc[i] >= data['Lower_KC'].iloc[i] + 1:
            if data['Low'].iloc[i-1] <= data['Lower_KC'].iloc[i-1] - 1 or \
            data['Low'].iloc[i] <= data['Lower_KC'].iloc[i] - 1:
