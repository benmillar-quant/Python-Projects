                        ...
                        if j >= len(data):
                            ...
                            if data['High'].iloc[j] >= data['High'].iloc[i] + spread:
                                if (data['High'].iloc[i] + spread - (data['Low'].iloc[i] - spread)) <= max_risk:
                                    ...
                                    data.at[data.index[j], 'status'] = 'Buy Trade Opened'
        ...
        if trade['active']:
            ...
            if i > trade_open_index:
                if data['Low'].iloc[i] <= trade['stop_loss']:
                    ...
                    data.at[data.index[trade_open_index], 'status'] = 'Buy Trade Closed'
...
#                if data['High'].iloc[i] >= trade['take_profit_target']:
 #                   trade['active'] = False
  #                  pnl = trade['take_profit_target'] - trade['entry_price']
   #                 cumulative_pnl += pnl
    #                total_wins += 1

     #               data.at[data.index[trade_open_index], 'status'] = 'Buy Trade Closed via TP'
      ...
      #                      data.at[data.index[trade_open_index], 'total_losses'] = total_losses
       #                     data.at[data.index[trade_open_index], 'total_wins'] = total_wins
                ...
                if data['Time'].iloc[i] == eod:
                    ...
                    data.at[data.index[trade_open_index], 'status'] = 'Buy Trade Closed at EOD'
