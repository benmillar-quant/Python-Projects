...
data = pd.read_csv("SNP500_shortfiltered_2024_data.csv")
...
for i in range(len(data)):
    ...
    if within_trade_hours and not trade_open and daily_losses < 3:
        ...
        if data['Close'].iloc[i] < data['Open'].iloc[i] and data['Close'].iloc[i] <= data['Upper_KC'].iloc[i] - 0.1:
            if data['High'].iloc[i] >= data['Upper_KC'].iloc[i] + 0.1:
      ...
      #                      data.at[data.index[i], 'PnL'] = pnl
       #                     data.at[data.index[i], 'total_losses'] = total_losses
        #                    data.at[data.index[i], 'total_wins'] = total_wins
            ...
            # if data['Low'].iloc[i] <= data['EMA'].iloc[trade_open_index]:
            #     # Take-profit condition met based on EMA
            #     trade_outcome = 'Win'
            #     pnl = sell_entry_price - data['EMA'].iloc[trade_open_index]
            #     trade_open = False
            #     exit_time = data['Time'].iloc[i]
            #     cumulative_pnl += pnl
            #     total_wins += 1
            ...
            #     # Update data DataFrame with trade exit details
            #     data.at[data.index[i], 'trade_open'] = 'Sell Trade Closed via TP (EMA)'
...
print("Cumulative PnL: {:.2f} Pts".format(cumulative_pnl * 10))
