...
for i in range(len(data)):
    ...
    if current_date != previous_date:
        ...
        # Set fixed trade opening hours
        trade_open_hours = [("07:30:00", "08:30:00"), ("09:30:00", "14:57:00")]
    ...
    if within_trade_hours and not trade_open and daily_losses < 3:
        ...
        # # SELL SIGNAL CONDITIONS ONLY USING FOR SCALPING AT RED LINE WITH FIXED TP AT EMA
        # if data['Close'].iloc[i] < data['Open'].iloc[i] and data['Close'].iloc[i] <= data['Upper_KC'].iloc[i] - 1:
        #     if data['High'].iloc[i] >= data['Upper_KC'].iloc[i] + 1:
        #         midpoint = (data['Upper_KC'].iloc[i] + data['EMA'].iloc[i]) / 2
        #         if data['Low'].iloc[i] >= midpoint:
        #             fixed_take_profit = data['EMA'].iloc[i]  # Store the EMA value from the signal bar
        #             for j in range(i + 1, i + 4):
        #                 if j >= len(data):
        #                     break
        #                 else:
        ...
        #                             take_profit_target = fixed_take_profit  # Use fixed EMA value
         ...
         #       data.at[data.index[i], 'risk'] = risk
          #      data.at[data.index[i], 'total_losses'] = total_losses
           #     data.at[data.index[i], 'total_wins'] = total_wins
            ...
            #     trade_outcome = 'Win'
            #     pnl = sell_entry_price - take_profit_target
            #     trade_open = False
            #     exit_time = data['Time'].iloc[i]
            #     cumulative_pnl += pnl
            #     total_wins += 1
            ...
            # # Check for Lower KC-based take profit
            # if data['Low'].iloc[i] <= data['Lower_KC'].iloc[trade_open_index]:
            #     # Take-profit condition met based on Lower KC
            #     trade_outcome = 'Win'
            #     pnl = sell_entry_price - data['Lower_KC'].iloc[trade_open_index]
            #     trade_open = False
            #     exit_time = data['Time'].iloc[i]
            #     cumulative_pnl += pnl
            #     total_wins += 1
            ...
            #     # Update data DataFrame with trade exit details
            #     data.at[data.index[i], 'trade_open'] = 'Sell Trade Closed via TP (Lower KC)'
            #     data.at[data.index[i], 'exit_price'] = data['Lower_KC'].iloc[trade_open_index]
            #     data.at[data.index[i], 'exit_time'] = exit_time
            #     data.at[data.index[i], 'outcome'] = trade_outcome
            #     data.at[data.index[i], 'PnL'] = pnl
            #     data.at[data.index[i], 'entry_price'] = sell_entry_price
