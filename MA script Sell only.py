...
for i in range(len(data)):
    ...
    if can_open_new_trade:
        #### TRADE TYPE A ###
        # Trade A: Daily bar closes below 200 Period MA
        if data['Close'].iloc[i] < data['Open'].iloc[i] and data['Close'].iloc[i] <= data['200_MA'].iloc[i] - 1:
            if data['Close'].iloc[i-1] >= data['200_MA'].iloc[i-1] + 1 or data['High'].iloc[i] >= data['200_MA'].iloc[i] + 1:
                    ...
                    if data['Low'].iloc[j] <= data['Low'].iloc[i] - spread:
                        ...
                        if (data['High'].iloc[i] + spread - (data['Low'].iloc[i] - spread)) <= max_risk:
                            ...
                            trade_details = {
                                ...
                                'take_profit_target': data['Low'].iloc[i] - (data['ATR'].iloc[i] * atr_take_profit_multiplier),
                                'trade_open_idx': j
                            }
                            ...
                            data.at[j, 'status'] = 'Sell Trade A Opened'
        ...
        #### TRADE TYPE B ###
        # Trade B: Daily bar closes below 50 Period MA while also below the 200 Period MA
                ...
                for j in range(i+1, i+4):
                    if data['Low'].iloc[j] <= data['Low'].iloc[i] - spread:
                        ...
                        if (data['High'].iloc[i] + spread - (data['Low'].iloc[i] - spread)) <= max_risk:
                            ...
                            data.at[j, 'status'] = 'Sell Trade B Opened'
                            data.at[j, 'entry_price'] = trade_details['entry_price']
                            data.at[j, 'stop_loss'] = trade_details['stop_loss']
                            data.at[j, 'risk'] = trade_details['risk']
                            data.at[j, 'take_profit_target'] = trade_details['take_profit_target']
    ...
    for trade in active_trades:
        ...
        if trade['active'] and trade['trade_type'] == 'A':
            if i > trade['trade_open_idx']:
                ...
                if not trade['stop_loss_moved_to_be'] and data['Low'].iloc[i] <= trade['entry_price'] - data['ATR'].iloc[i] * atr_stop_loss_multiplier:
                ...
                # Check for take profit condition based up a the daily bar closing back above the 200 period MA
                # if data['Close'].iloc[i] > data['Open'].iloc[i] and data['Close'].iloc[i] >= data['200_MA'].iloc[i] + 1:
        ...
        if trade['active'] and trade['trade_type'] == 'B':
            if i > trade['trade_open_idx']:
                ...
                # Check for take profit condition based up a the daily bar closing back above the 50 period MA
