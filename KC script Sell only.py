...
data = pd.read_csv("DJIA_Data_SELL_CSV.csv")
...
### DISPLAYING THE PRICE BAR CHART AND KELTER CHANNEL LINES ###
        ...
        fig = go.Figure(data=[go.Candlestick(x=group.index,
                                             ...
                                             close=group['Close'],
                                             name="Candlesticks"),
                              go.Scatter(x=ema.index, y=upper_kc, mode='lines', name='Upper KC', line=dict(color='blue')),
                              go.Scatter(x=ema.index, y=ema, mode='lines', name='EMA', line=dict(color='green')),
                              go.Scatter(x=ema.index, y=lower_kc, mode='lines', name='Lower KC', line=dict(color='red'))])
...
for i in range(len(data)):
    ...
    if current_date != previous_date:
        ...
        if last_daily_atr > 375:
            ...
            trade_open_hours = [("09:30:00", "10:30:00"), ("15:00:00", "15:57:00")]
    ...
    if within_trade_hours and not trade_open and daily_losses < 3:
        ...
        if data['Close'].iloc[i] < data['Open'].iloc[i] and data['Close'].iloc[i] <= data['Upper_KC'].iloc[i] - 1:
            if data['High'].iloc[i-1] >= data['Upper_KC'].iloc[i-1] + 1 or \
            data['High'].iloc[i] >= data['Upper_KC'].iloc[i] + 1:
