...
data = pd.read_csv("NASDAQ_shortfiltered_2023_data.csv")
...
for i in range(len(data)):
    ...
    if within_trade_hours and not trade_open and daily_losses < 3:
        ...
        if data['Close'].iloc[i] < data['Open'].iloc[i] and data['Close'].iloc[i] <= data['Upper_KC'].iloc[i] - 0.5:
            if data['High'].iloc[i] >= data['Upper_KC'].iloc[i] + 0.5:
...
print("Cumulative PnL: {:.2f} Pts".format(cumulative_pnl * 2))
