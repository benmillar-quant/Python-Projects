...
for i in range(len(data)):
    ...
    if is_open_time and daily_losses < 3:
        ...
        if can_open_new_trade:
            ...
            if (
                overbought
                and data['RSI_21'].iloc[i-1] >= oversold_level
                and data['RSI_21'].iloc[i]   <= oversold_level
            ):
    ...
    for t in active_trades:
        ...
        # Stop-loss for short
        if data['High'].iloc[i] >= t['stop_loss']:
            t['active'] = False
        ...
        # # # Move SL to BE for short
        if not t['stop_loss_moved_to_be'] and data['Low'].iloc[i] <= t['entry_price'] - stop_loss_to_BE:
            t['stop_loss'] = t['entry_price']
            t['stop_loss_moved_to_be'] = True
            data.at[t['trade_open_idx'],'stop_loss'] = t['entry_price']

        # RSI-based TP for short
        if data['RSI_21'].iloc[i] >= overbought_level:
            tp_met = False
            for j in range(i+1, len(data)):
                ...
                if data['High'].iloc[j] >= data['High'].iloc[i] + spread:
                    pot = t['entry_price'] - (data['High'].iloc[i] + spread)
                    if pot >= 1:
                        tp_met = True
                        t['exit_price'] = data['High'].iloc[i] + spread
                        break
            if tp_met:
                t['active'] = False
                t['stop_loss_moved_to_be'] = False
