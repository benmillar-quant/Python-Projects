...
data = pd.read_csv("SNP500_longfiltered_2024_data.csv")
...
for i in range(len(data)):
    ...
    if within_trade_hours and not trade_open and daily_losses < 3:
        ...
        if data['Close'].iloc[i] > data['Open'].iloc[i] and data['Close'].iloc[i] >= data['Lower_KC'].iloc[i] + 0.1:
            if data['Low'].iloc[i] <= data['Lower_KC'].iloc[i] - 0.1:
